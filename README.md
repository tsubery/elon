...
Like any distributed computer system, Bitcoin has certain capacities that can be consumed by attackers in order to render services [unavailable](https://en.wikipedia.org/wiki/Denial-of-service_attack) for normal usage. These limitations are kept intentionally low in order to make the software more "decentralized" for marketing purposes, making the network vulnerable to long durations of service outages.
